...
class PrivatePostTests(TestCase):
    ...
    def test_post_detail_view(self):
        ...
        response = self.client.get(
            reverse("cat_network:post-detail", kwargs={"pk": post.id})
        )
        self.assertEqual(response.status_code, 200)
...
class PostLikeTests(TestCase):
    ...
    def test_toggle_like_post(self):
        response = self.client.post(
            reverse("cat_network:toggle-like"), {"post_id": self.post.id}
        )
        self.assertEqual(response.status_code, 302)
...
class CommentTests(TestCase):
    ...
    def test_comment_create_view(self):
        ...
        response = self.client.post(
            reverse("cat_network:comment-create", kwargs={"pk": self.post.id}),
            data=form_data,
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse("cat_network:comment-list", kwargs={"pk": self.post.id})
        )
    ...
    def test_comment_update_view(self):
        comment = Comment.objects.create(
            text="Original comment", author=self.user, post=self.post
        )
        form_data = {
            "text": "Updated comment",
        }
        response = self.client.post(
            reverse("cat_network:comment-update", kwargs={"pk": comment.id}),
            data=form_data,
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
    ...
    def test_comment_delete_view(self):
        comment = Comment.objects.create(
            text="Comment to delete", author=self.user, post=self.post
        )
        response = self.client.post(
            reverse("cat_network:comment-delete", kwargs={"pk": comment.id})
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse("cat_network:comment-list", kwargs={"pk": self.post.id})
...
class CatUserTests(TestCase):
    ...
    def test_cat_user_list_view(self):
        ...
        CatUser.objects.create(
            username="catuser2", first_name="Another", last_name="User"
        )
    ...
    def test_cat_user_detail_view(self):
        cat_user = CatUser.objects.create(
            username="catuser", first_name="Cat", last_name="User"
        )
        response = self.client.get(
            reverse("cat_network:cat-detail", kwargs={"pk": cat_user.id})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_cat_user_create_view(self):
        ...
        self.assertRedirects(
            response, reverse("cat_network:cat-detail", kwargs={"pk": new_user.pk})
        )
...
class CatUserUpdateTests(TestCase):
    ...
    def test_cat_user_update_view(self):
        ...
        response = self.client.post(
            reverse("cat_network:cat-update", kwargs={"pk": user.id}), data=form_data
        )
        ...
        self.assertRedirects(
            response, reverse("cat_network:cat-detail", kwargs={"pk": updated_user.pk})
        )
...
class CatUserFollowersTests(TestCase):
    def setUp(self):
        ...
        self.client.force_login(self.user)
        self.cat = CatUser.objects.create(
            username="catuser", first_name="Cat", last_name="User"
        )
    ...
    def test_cat_user_followers_view(self):
        response = self.client.get(
            reverse("cat_network:cat-followers", kwargs={"pk": self.cat.id})
        )
        self.assertEqual(response.status_code, 200)
...
class FollowCatUserTests(TestCase):
    ...
    def test_follow_catuser(self):
        response = self.client.post(
            reverse("cat_network:follow-catuser", kwargs={"pk": self.cat.id})
        )
        self.assertEqual(response.status_code, 302)
